function [channels, rays, is_ray_blocked]  = get_channels_from_model(model,src,dst,t_res,enable_printing,src_orient,dst_orient, gain_pattern)
...
%         dp_ang = dp_ang + 360;
%     end
%     src_atten = ceil(src_orient-dp_ang);
%     dst_atten = ceil(dst_orient-dp_ang);
%     if src_atten <= 0
%         src_atten = src_atten + 360;
%     end
%     if dst_atten <= 0
%         dst_atten = dst_atten + 360;
%     end
%     src_atten = db2mag(gain_pattern(src_atten));
%     dst_atten = db2mag(gain_pattern(dst_atten));
    
    ray_status=[0,0];
    for i=1:length(model.reflectors)
        %[p,t]=is_reflect_edit_fast2(src,dst, model.reflectors{i});  % TODO, Add 3D  
        [p,t]=is_reflect_edit_3d(src,dst, model.reflectors{i});  % TODO, Add 3D  
        if(t)
    ...
    for i=1:length(rays)
        if(is_ray_clear(i) ~= 0)
            cur_d = 0;
            for j=1:size(rays{i},1)-1
                cur_d = cur_d + norm(rays{i}(j+1,[1,2,3])-rays{i}(j,[1,2,3]));
            end
            if abs(cur_d-dir_path_len) > d_res % If 
                continue
            end
            for j=1:length(lambda)
                channels(j) = channels(j) + is_ray_clear(i)*amps(j)*1/(cur_d/lambda(j))*exp(-1j*2*pi/lambda(j)*cur_d);% * src_atten*dst_atten;
                %channels(j) = channels(j) + is_ray_clear(i)*amps(j)*exp(-1j*2*pi/lambda(j)*cur_d);% * src_atten*dst_atten;
            end
        end
    end    
end
