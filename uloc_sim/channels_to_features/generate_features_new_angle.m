function features = generate_features_new_angle(channels,ap,theta_vals,d_vals,d1,d2,opt,model)
...
features = zeros(2*n_ap*n_ap,length(d2),length(d1));
feature_idx=1;
...
for j=1:n_ap
    ...
    for l = 1:n_ap-1
        ...
        features(feature_idx,:,:) = real(Prel_out);
        features(feature_idx+1,:,:) = imag(Prel_out);
    ...
    features(feature_idx,:,:) = (real(P_out));
    features(feature_idx+1,:,:) = (imag(P_out));
